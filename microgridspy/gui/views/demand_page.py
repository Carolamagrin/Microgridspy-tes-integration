...
def demand_assessment() -> None:
    ...
    with st.expander("Import Thermal Cooling Demand", expanded=False):
        st.write("You can upload or define the thermal cooling demand (in kW of cooling).")

        thermal_input_type = st.radio(
            "Choose thermal demand input method:",
            ["None (set to zero)", "Upload CSV", "Constant value"],
            key="thermal_input_type"
        )

        # No thermal demand (will be handled in initialize_thermal_demand)
        if thermal_input_type == "None (set to zero)":
            st.session_state.thermal_demand_path = None
            st.info("Thermal demand will be set to zero automatically during model initialization.")

        # Upload CSV
        elif thermal_input_type == "Upload CSV":
            uploaded_file, delimiter_value, decimal_value = csv_upload_interface("thermal")
            if uploaded_file is not None:
                csv_data = load_csv_data(uploaded_file, delimiter_value, decimal_value)
                if csv_data is not None:
                    st.dataframe(csv_data.head())
                    ...
                    if csv_data.shape[1] < time_horizon:
                        st.error(f"The uploaded file must contain at least {time_horizon} yearly columns.")
                    else:
                        if st.button("Save Thermal Demand CSV", key="save_thermal_csv"):
                            save_demand_data(
                                csv_data,
                                filename="thermal_demand.csv",
                                project_name=project_name,
                                start_year=start_year
                            )
                            st.session_state.thermal_demand_path = "thermal_demand.csv"
                            st.success("Thermal demand saved successfully.")

        # Constant value input
        elif thermal_input_type == "Constant value":
            const_value = st.number_input("Constant cooling demand [kW_cool]:", min_value=0.0)
            if st.button("Save Constant Thermal Demand", key="save_thermal_constant"):
                thermal_profile = np.full(365 * 24, const_value)  # hourly for 365 days
                demand_df = multi_years_demand(thermal_profile, time_horizon)
                save_demand_data(
                    demand_df,
                    filename="thermal_demand.csv",
                    project_name=project_name,
                ...
                st.session_state.thermal_demand_path = "thermal_demand.csv"
                st.success("Constant thermal demand saved successfully.")
    ...
    with st.expander("⏯ Simulate High-Resolution Load Profiles using RAMP Software", expanded=False):
    ...
    with col2:
        if st.button("Next"):
            st.session_state.page = "Cooling Technology"
            st.rerun()
