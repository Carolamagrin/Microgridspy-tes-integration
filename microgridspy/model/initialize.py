...
def initialize_demand(sets: xr.Dataset) -> xr.DataArray:
    ...
    try:
        ...
        zero_data = np.zeros((num_scenarios, num_periods, num_years))
        return xr.DataArray(
            data=zero_data,
...
def initialize_thermal_demand(sets: xr.Dataset) -> xr.DataArray:
    """
    Load the thermal demand time series into an xarray DataArray.
    """
    ...
    try:
        thermal_df = read_csv_data(PathManager.THERMAL_DEMAND_FILE_PATH)

    except (FileNotFoundError, pd.errors.EmptyDataError, pd.errors.ParserError) as e:
        # se il file non è trovato mi restituisce zeri
        zero_data = np.zeros((num_scenarios, num_periods, num_years))
    ...
    # Se il file esiste controlla vada bene
    num_columns = thermal_df.shape[1]

    if num_columns < num_years:
        raise RuntimeError(
            f"The number of years in the thermal demand file ({num_columns}) "
            f"is less than the number of years in the time horizon ({num_years})."
        )

    elif num_columns > num_years:
        thermal_df = thermal_df.iloc[:, :num_years]
        st.warning(
            f"Number of years detected in the thermal demand file ({num_columns}) "
            f"is higher than the number of years in the time horizon ({num_years}). "
            f"The data will be truncated."
        )

    thermal_data = thermal_df.values.reshape(num_scenarios, num_periods, num_years)
...
def initialize_project_parameters(data: ProjectParameters, sets: xr.Dataset) -> xr.Dataset:
    ...
    project_parameters['COP_DIRECT'] = xr.DataArray(
        data.project_settings.cop_direct,
        dims=[],
        name='Coefficient of Performance (Cooling Compressor)')
...
def initialize_compressor_parameters(data: ProjectParameters, sets: xr.Dataset) -> xr.Dataset:
    """
    Initialize the compressor parameters as xr.Dataset.
    """
    compressor_parameters = {
        
        'COMPRESSOR_SPECIFIC_INVESTMENT_COST': xr.DataArray(
            data.compressor_params.compressor_specific_investment_cost,
            dims=[],
            name='Compressor Specific Investment Cost'),

        'COMPRESSOR_SPECIFIC_OM_COST': xr.DataArray(
            data.compressor_params.compressor_specific_om_cost,
            dims=[],
            name='Compressor Specific O&M Cost'),

        'COMPRESSOR_LIFETIME': xr.DataArray(
            data.compressor_params.compressor_lifetime,
            dims=[],
            name='Compressor Lifetime (years)'),

    }

    return xr.Dataset(compressor_parameters)

def initialize_tes_parameters(data: ProjectParameters, sets: xr.Dataset) -> xr.Dataset:
    tes_params = {
        "TES_CAPACITY": xr.DataArray(data.tes_params.tes_capacity, dims=[]),
        "TES_INITIAL_SOC": xr.DataArray(data.tes_params.tes_initial_soc, dims=[]),
        "TES_MAX_CHARGE_RATE": xr.DataArray(data.tes_params.tes_max_charge_rate, dims=[]),
        "TES_MAX_DISCHARGE_RATE": xr.DataArray(data.tes_params.tes_max_discharge_rate, dims=[]),
        "TES_STORAGE_EFFICIENCY": xr.DataArray(data.tes_params.tes_storage_efficiency, dims=[]),
        "TES_COP": xr.DataArray(data.tes_params.tes_cop, dims=[]),
        "TES_Q_PER_KG": xr.DataArray(data.tes_params.tes_q_per_kg, dims=[]),
        "TES_SPECIFIC_INVESTMENT_COST": xr.DataArray(
            data.tes_params.tes_specific_investment_cost, dims=[]
        ),
        "TES_SPECIFIC_OM_COST": xr.DataArray(
            data.tes_params.tes_specific_om_cost, dims=[]
        ),
        "TES_LIFETIME": xr.DataArray(
            data.tes_params.tes_lifetime, dims=[]
        ),
        "TES_COMPRESSOR_SPECIFIC_INVESTMENT_COST": xr.DataArray(
            data.tes_params.tes_compressor_specific_investment_cost, dims=[]
        ),
        "TES_COMPRESSOR_SPECIFIC_OM_COST": xr.DataArray(
            data.tes_params.tes_compressor_specific_om_cost, dims=[]
        ),
        "TES_COMPRESSOR_LIFETIME": xr.DataArray(
            data.tes_params.tes_compressor_lifetime, dims=[]
        ),
...
def initialize_grid_parameters(data: ProjectParameters, sets: xr.Dataset) -> xr.Dataset:
    ...
    grid_parameters = {
        ...
        #TODO: connect to the max power grid param
