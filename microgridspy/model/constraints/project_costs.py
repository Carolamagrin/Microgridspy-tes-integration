...
def add_investment_cost(
    ...
    # Compressore raffreddamento diretto
    if settings.advanced_settings.use_compressor:
        investment_cost += (
            var['compressor_capacity']
            * param['COMPRESSOR_SPECIFIC_INVESTMENT_COST']
        )
    ...
    if settings.advanced_settings.use_tes:
        investment_cost += (
            ...
            * param['TES_COMPRESSOR_SPECIFIC_INVESTMENT_COST']
        )
    ...
    if settings.advanced_settings.use_tes:
        investment_cost += (
            param['TES_CAPACITY']
            * param['TES_SPECIFIC_INVESTMENT_COST']
        )
...
def add_fixed_om_cost(
    ...
    if actualized:
        for year in years:
            ...
            # Compressor diretto O&M cost (actualized)
            if settings.advanced_settings.use_compressor:
                om_cost += (
                    var['compressor_capacity']
                    * param['COMPRESSOR_SPECIFIC_INVESTMENT_COST']
                    * param['COMPRESSOR_SPECIFIC_OM_COST']
                    * discount_factor.sel(years=year)
                )

            # TES compressor O&M cost (actualized)
            if settings.advanced_settings.use_tes:
                om_cost += (
                    var['tes_compressor_capacity']
                    * param['TES_COMPRESSOR_SPECIFIC_INVESTMENT_COST']
                    * param['TES_COMPRESSOR_SPECIFIC_OM_COST']
                    * discount_factor.sel(years=year)
                )

            # TES O&M cost (actualized)
            if settings.advanced_settings.use_tes:
                om_cost += (
                    param['TES_CAPACITY']
                    * param['TES_SPECIFIC_OM_COST']
                    * discount_factor.sel(years=year)
                )
        ...
        for year in sets.years.values:
            ...
            # Compressor O&M cost - raffreddamento diretto
            if settings.advanced_settings.use_compressor:
                om_cost += (
                    var['compressor_capacity']
                    * param['COMPRESSOR_SPECIFIC_INVESTMENT_COST']
...
def add_salvage_value(
    ...
    if settings.advanced_settings.use_compressor:
        remaining_compressor_lifetime = where(
            param['COMPRESSOR_LIFETIME'] - project_duration > 0,
            param['COMPRESSOR_LIFETIME'] - project_duration,
            0
        )
        ...
        salvage_value += (
            var['compressor_capacity']
            * param['COMPRESSOR_SPECIFIC_INVESTMENT_COST']
            * (remaining_compressor_lifetime / param['COMPRESSOR_LIFETIME'])
            * discount_factor
        )
    
    # Salvage compressore TES
    if settings.advanced_settings.use_tes:
        remaining_tes_compressor_lifetime = where(
            param['TES_COMPRESSOR_LIFETIME'] - project_duration > 0,
            param['TES_COMPRESSOR_LIFETIME'] - project_duration,
            0
        )
        ...
        salvage_value += (
            var['tes_compressor_capacity']
            * param['TES_COMPRESSOR_SPECIFIC_INVESTMENT_COST']
            * (remaining_tes_compressor_lifetime / param['TES_COMPRESSOR_LIFETIME'])
            * discount_factor
        )

    # TES salvage value
    if settings.advanced_settings.use_tes:
        remaining_tes_lifetime = where(
            param['TES_LIFETIME'] - project_duration > 0,
            param['TES_LIFETIME'] - project_duration,
            0
        )
        ...
        salvage_value += (
            param['TES_CAPACITY']
            * param['TES_SPECIFIC_INVESTMENT_COST']
            * (remaining_tes_lifetime / param['TES_LIFETIME'])
            * discount_factor
        )
