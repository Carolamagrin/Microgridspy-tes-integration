...
def add_energy_balance_constraints(
    ...
    for year in sets.years.values:
        ...
        if has_compressor and has_tes:
            ...
            extra_electric_demand = var['compressor_electric_consumption'].sel(years=year)
        elif has_tes:
            extra_electric_demand = var['tes_electric_consumption'].sel(years=year)
        else:
            # caso in cui non ci sono né compressore né TES: extra = 0
        ...
        if has_compressor or has_tes:
            ...
            model.add_constraints(
                cooling_direct + cooling_from_tes >= thermal_demand,
...
def add_lost_load_constraint(
    ...
    years = sets.years.values
    use_compressor = settings.advanced_settings.use_compressor
    ...
    for year in years:
        ...
        if use_compressor and use_tes:
            ...
            served_by_cooling = var['compressor_electric_consumption'].sel(years=year)
        elif use_tes:
            served_by_cooling = var['tes_electric_consumption'].sel(years=year)
        else:
